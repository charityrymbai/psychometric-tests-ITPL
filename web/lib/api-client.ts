...
export const DIRECT_SERVER_URL = process.env.NEXT_PUBLIC_BACKEND_BASE_URL || 'http://localhost:3002';
...
async function fetchWithErrorHandling(url: string, options: RequestInit = {}) {
  try {
    ...
    if (error instanceof TypeError && error.message === 'Failed to fetch') {
      ...
      console.error('API_BASE_URL is:', DIRECT_SERVER_URL);
    }
    throw error;
  }
}
...
export const userApi = {
  // Create a new user - requires a numeric ID parameter for the database
  createUser: async (tempId: string, userData: { name: string, class: number }) => {
    ...
    // Ensure tempId is a valid number for the database
    if (isNaN(Number(tempId))) {
      console.warn('Warning: tempId is not a valid number, this may cause database errors');
    }
    
    try {
      // Try the plural endpoint first as it's more common in REST APIs
      try {
        const response = await fetchWithErrorHandling(`${DIRECT_SERVER_URL}/user/${tempId}`, {
          method: 'POST',
  ...
  getUser: async (userId: string) => {
    ...
    try {
      try {
        return await fetchWithErrorHandling(`${DIRECT_SERVER_URL}/user/${userId}`);
      } catch (error) {
        console.log('Failed with first endpoint, trying again...');
        return await fetchWithErrorHandling(`${DIRECT_SERVER_URL}/user/${userId}`);
      }
    } catch (error) {
  ...
  updateUser: async (userId: string, userData: { name?: string, class?: number }) => {
    try {
      ...
      console.log('Failed with /users endpoint, trying /user...');
      return fetchWithErrorHandling(`${DIRECT_SERVER_URL}/user/${userId}`, {
        method: 'PUT',
        body: JSON.stringify(userData),
      });
    }
